...
export function mergeBrokerSearchCompanies(lists: BrokerSearchCompany[][]): BrokerSearchCompany[] {
  const byCompany = new Map<string, BrokerSearchCompany>()
  ...
  for (const list of lists) {
    for (const company of list) {
      ...
      const seen = new Set((existing.results ?? []).map((row) => (row.name ?? "").trim()).filter(Boolean))
      for (const row of company.results ?? []) {
        const name = (row.name ?? "").trim()
        if (!name || seen.has(name)) continue
        existing.results = [...(existing.results ?? []), row]
        seen.add(name)
      }
    }
  }
...
/** Derive API query fragments when the user typed a server name instead of a company. */
export function deriveBrokerSearchVariants(query: string): string[] {
  const trimmed = query.trim()
  const variants: string[] = []
  const add = (value: string) => {
    const candidate = value.trim()
    if (candidate.length >= 4 && !variants.includes(candidate)) {
      variants.push(candidate)
    }
  }

  add(trimmed)
  add(withoutTrailingNumber(trimmed))
