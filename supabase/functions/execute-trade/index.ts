...
type BrokerRow = JsonRecord & {
  ...
  /** When not true, ignore signal_channel_ids (copy all user Telegram channels). */
...
function normalizeAiSettings(raw: unknown): {
  ...
  /** `equity` (default): use equity then balance. `balance`: use balance even when equity exists. */
  risk_basis: "equity" | "balance"
  /**
   * `linear` (default): lot = default_lot × (account_basis × risk% / ref_equity × 1%) — scales smoothly with account size.
   * `margin`: lot = risk_budget_usd / RequiredMargin(1 lot) when API returns valid margin.
   */
...
async function resolveLotAndPips(
  ...
  if (mode === "manual") {
    if (manual.risk_mode === "dynamic_balance_percent" && metaId) {
      ...
      if (basis != null && basis > 0) {
        // Conservative base-lot approximation for manual percent mode.
    ...
    return {
      ...
      sizingLog: {
        source: "manual_mode",
        risk_mode: manual.risk_mode,
        trade_style: manual.trade_style,
        enabled_tp_lots: enabledTpLots,
        broker_min_lot_floor: brokerLotFloor,
      },
    }
  }
  ...
  // Default AI path: linear in account basis vs reference_equity (1% of ref_equity = anchor for default_lot).
...
/** Parser/LLM sometimes puts English words in `symbol` on follow-ups — do not use for ticket matching. */
...
/** Returns normalized symbol for matching, or null if the token should be ignored (treat as unspecified). */
...
/** Latest open Copier trade on this broker whose originating signal was from this Telegram channel. */
...
/** Margin in account currency for exactly 1 lot at the given side/price (Metatrader API). */
...
/** OrderSend responses often include openPrice / dealInternalIn.openPrice; parsed entry may be null for market orders. */
...
/** Resolve open / entry price from live positions when `trades.entry_price` was never stored (e.g. market entry). */
...
async function executeOneBroker(
  ...
  if (mode === "manual") {
    ...
    if (manual.use_predefined_sl_pips && effectiveParsed.sl == null) {
      ...
      if (Number.isFinite(anchor) && anchor > 0) {
        const dir = effectiveParsed.action === "sell" ? 1 : -1
        effectiveParsed.sl = anchor + (dir * (manual.predefined_sl_pips / 100))
      }
    }
  }
    ...
    if (effectiveParsed.action === "buy" || effectiveParsed.action === "sell") {
      ...
      if (mode === "manual" && !manual.add_new_trades_to_existing) {
        ...
        if (existingForSymbol?.metaapi_order_id) {
          ...
          if (
            manual.close_on_opposite_signal &&
            String(existingForSymbol.direction ?? "").toLowerCase() !== effectiveParsed.action
          ) {
      ...
      if (continuation?.trade?.metaapi_order_id) {
        ...
        await supabase
          ...
          .update({
            ...
            entry_price: effectiveParsed.entry_price ?? effectiveParsed.entry_zone_low ?? existing.entry_price ?? null,
