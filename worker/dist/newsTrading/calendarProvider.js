...
function parseImpact(raw) {
    ...
    if (s === 'high' || s === '3' || s.includes('high'))
        return 'high';
    if (s === 'medium' || s === '2' || s.includes('med'))
        return 'medium';
    return 'low';
}
...
async function getCalendarEventsCached(now = new Date()) {
    ...
    try {
        const { statusCode, body } = await (0, undici_1.request)(url.toString(), { method: 'GET', maxRedirections: 2 });
