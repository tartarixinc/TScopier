...
function classifyTradableInstrument(symbol) {
    ...
    if (/^(US|DE|UK|AU|JP|HK|EU|FR|ES|CH|CN|IN)\d{2,4}$/.test(s))
        return 'index';
    if (/^(NAS|SPX|DJ|DAX|FTSE|NIK|HSI|STOXX)\d{0,4}$/i.test(s) && /\d/.test(s))
        return 'index';
...
function isTradableInstrumentSymbol(symbol) {
...
function extractTradableSymbolFromMessage(raw) {
    ...
    if (onFor?.[1]) {
        ...
        if (/\b(GOLD|XAU)\b/.test(subUp))
            return 'XAUUSD';
        if (/\b(SILVER|XAG)\b/.test(subUp))
            return 'XAGUSD';
...
function sanitizeParsedSymbol(symbol) {
