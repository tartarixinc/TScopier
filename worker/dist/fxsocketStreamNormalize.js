...
exports.normalizeFxsocketWsMessage = normalizeFxsocketWsMessage;
...
function isPendingOrderRow(o) {
    const kind = String(o.kind ?? o.Kind ?? '').toLowerCase();
    if (kind === 'pending' || kind === 'order')
        return true;
    if (kind === 'position' || kind === 'deal')
        return false;
...
function isMarketPositionRow(o) {
    if (isPendingOrderRow(o))
        return false;
    const kind = String(o.kind ?? o.Kind ?? '').toLowerCase();
    if (kind === 'position' || kind === 'deal')
        return true;
    ...
    if (symbol && lots != null && lots > 0 && (o.profit != null || o.Profit != null))
        return true;
    return false;
}
function isAccountSummaryRow(o) {
