...
class TradeExecutor {
    ...
    async prewarmBrokerCaches() {
        if (!this.prewarmSymbolsEnabled() || !(0, metatraderapi_1.hasMetatraderApiConfigured)())
            return;
    ...
    async cancelSignalEntryBrokerRowsForScope(scope, userId, logSignalId, reason) {
        ...
        for (const r of (seRows ?? [])) {
            ...
            if (api) {
                await (0, signalEntryPendingHelpers_1.cancelSignalEntryRowAtBroker)(this.supabase, api, r, reason);
            }
            else {
                await this.supabase
                    .from('signal_entry_pending_orders')
    ...
    async applyBasketSlTpRefresh(args) {
        const { signal, parsed, broker, channelKeywords, baseLot, params, symbol, uuid, strictEntryPrefetch, anchorSignalId, direction, logAction, mergeLinkMeta, } = args;
        const api = this.apiFor(broker);
        if (!api) {
            return {
                success: false,
        ...
        const refreshTpLevels = (parsed.tp ?? []).filter((t) => typeof t === 'number' && Number.isFinite(t) && t > 0);
        ...
        if (mergeFailed && !allLegsGhostOnBroker) {
            ...
            const { data: existingJob } = await this.supabase
                ...
                .maybeSingle();
            if (existingJob?.id) {
                await (0, basketSlTpReconcile_1.markBasketReconcileDone)(this.supabase, existingJob.id);
            }
        }
    ...
    async sendOrder(signal, parsed, op, broker, channelKeywords, pipelineT0, sendOpts) {
                ...
                try {
                    void this.supabase.from('trade_execution_logs').insert({
                        ...
                        request_payload: {
                            ...
                            parsed_tp_levels: (parsed.tp ?? []).filter((t) => typeof t === 'number' && Number.isFinite(t) && t > 0),
        ...
        if (insertRows.length > 0) {
            ...
            if (liveEntryFast) {
                ...
                const persist = await this.persistRangePendingLegRows(insertRows, persistLabel);
    ...
    async applyManagement(signal, parsed, brokers) {
        ...
        const pendingLegs = await (0, managementPendingLegs_1.loadRangePendingLegsInMgmtScope)(this.supabase, {
            userId: signal.user_id,
            brokerAccountIds,
