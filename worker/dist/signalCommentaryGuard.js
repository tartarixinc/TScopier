...
function hasExecutableTradeStructure(message) {
    ...
    if (/\b(?:buy|sell)\s+(?:at\s+)?@\s*\d/i.test(text))
        return true;
    if (/\b(?:buy|sell)\s+at\s+\d/i.test(text))
        return true;
    return false;
}
/** Market news / macro commentary that mentions instruments but is not a trade signal. */
function looksLikeMarketNewsOrCommentary(message) {
    const text = String(message ?? '').replace(/\s+/g, ' ').trim();
    if (!text)
        return false;
    if (hasExecutableTradeStructure(text))
        return false;
    if (/\bmarket\s+news\b/i.test(text))
        return true;
    if (/\bnews\s+update\b/i.test(text))
        return true;
    if (/\bmarket\s+update\b/i.test(text))
        return true;
    if (/\bmarket\s+(?:analysis|recap|commentary|outlook|report)\b/i.test(text))
        return true;
    if (/\b(?:weekly|daily)\s+(?:recap|outlook|roundup)\b/i.test(text))
        return true;
    if (/📰/.test(text))
        return true;
