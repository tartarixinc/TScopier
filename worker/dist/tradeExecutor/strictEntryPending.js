...
exports.placeStrictSignalEntryPending = placeStrictSignalEntryPending;
 ...
 */
async function placeStrictSignalEntryPending(ctx, prep, singleTpOverride) {
    const { signal, parsed, broker, manual, api, uuid, symbol, params, plan, capped, strictDeferred, commentPrefix, } = prep;
    ...
    const expiresAt = pendHours > 0
        ...
        : null;
    const pendingOp = se.isBuy ? 'BuyLimit' : 'SellLimit';
    ...
    try {
        ...
        const partialTpPlan = singleTpOverride && capped.length === 1 && plan.partialTps?.length ? plan.partialTps : null;
        ...
        console.error(`[tradeExecutor] strict entry broker OrderSend failed signal=${signal.id} broker=${broker.id} op=${pendingOp} price=${entryPx}: ${msg}`);
        try {
            await ctx.supabase.from('trade_execution_logs').insert({
                user_id: signal.user_id,
                signal_id: signal.id,
                ...
                status: 'failed',
                request_payload: { operation: pendingOp, entry_price: entryPx, symbol },
                error_message: msg,
            });
        }
        catch { /* best-effort */ }
