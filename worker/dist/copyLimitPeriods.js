...
function periodKeyFor(period, timeZone, at = new Date()) {
    if (period === 'overall')
        return 'all';
...
function periodStorageKey(period, periodKey) {
    return `${period}:${periodKey}`;
}
/** UTC instant range for SQL filters (inclusive start, exclusive end). */
function periodWindowUtc(period, timeZone, at = new Date()) {
...
function isoWeekMonday(weekYear, week) {
...
function getTimezoneOffsetMinutesSafe(timeZone, at) {
    try {
        const dtf = new Intl.DateTimeFormat('en-US', {
            timeZone,
            year: 'numeric',
        ...
        const asUtc = Date.UTC(Number(filled.year), Number(filled.month) - 1, Number(filled.day), Number(filled.hour), Number(filled.minute), Number(filled.second));
