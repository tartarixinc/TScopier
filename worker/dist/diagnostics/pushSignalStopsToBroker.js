...
function tpForLegIndex(tps, legIndex) {
    if (!tps.length)
        return null;
...
async function main() {
    ...
    const signalTps = (parsed.tp ?? [])
        ...
        .filter((t) => t != null);
    if (signalSl == null)
        throw new Error(`signal ${signalId} has no parsed SL`);
    ...
    for (const brokerId of brokerIds) {
        ...
        const tpLots = broker.manual_settings?.tp_lots;
        ...
        for (let i = 0; i < legs.length; i++) {
            ...
            if (targetTp == null && signalTps.length) {
                targetTp = (0, tpBucketDistribution_1.takeProfitForLegIndex)({
                    legIndex: i,
            ...
            if (targetTp == null && num(tr.tp) != null)
                targetTp = num(tr.tp);
