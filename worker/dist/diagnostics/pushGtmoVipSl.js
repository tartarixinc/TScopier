...
exports.resolveGtmoVipAnchorSignalId = resolveGtmoVipAnchorSignalId;
exports.resolveGtmoVipUserIds = resolveGtmoVipUserIds;
/**
...
async function resolveGtmoVipAnchorSignalId(userId) {
    ...
    let signalsQ = supabase
        ...
        .limit(50);
    if (userId)
        signalsQ = signalsQ.eq('user_id', userId);
    ...
    if (trErr)
        throw trErr;
    const legCountBySignal = new Map();
...
async function pushGtmoVipSlForUser(args) {
    ...
    console.log(`Pushing SL_ONLY → ${args.slOverride}`
        + `${args.fxsocketOnly ? '  [FxSocket-only brokers]' : ''}`
        + `  dryRun=${args.dryRun}\n`);
    await (0, pushSignalStopsToBroker_1.runPushSignalStops)({
...
async function main() {
    ...
    if (allGtmoUsers) {
        ...
        if (!userIds.length)
            throw new Error('No GTMO VIP subscribers with open legs');
