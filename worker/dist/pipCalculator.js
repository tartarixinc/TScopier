...
exports.pipValueForLots = pipValueForLots;
const pipMath_1 = require("./pipMath");
const FX_DEFAULT_CONTRACT_SIZE = 100000;
const XAU_DEFAULT_CONTRACT_SIZE = 100; // ounces
const XAG_DEFAULT_CONTRACT_SIZE = 5000; // ounces
const INDEX_DEFAULT_CONTRACT_SIZE = 1;
const CRYPTO_DEFAULT_CONTRACT_SIZE = 1;
const ENERGY_DEFAULT_CONTRACT_SIZE = 1000; // barrels
 ...
 */
function inferQuoteCurrency(symbol, klass) {
    if (klass !== 'fx_major' && klass !== 'fx_jpy' && klass !== 'metal')
        return null;
 ...
 */
function resolveContractSize(klass, brokerSize) {
 ...
 */
function pipPriceFor(symbol, klass, point, digits) {
 ...
 * math from `point × 10`.
 */
function pipCalculator(symbol, point, digits, contractSize) {
    const klass = (0, pipMath_1.classifySymbol)(symbol);
 ...
 */
function pipValueForLots(quote, lots) {
    const n = Number(lots);
    if (!Number.isFinite(n) || n <= 0)
        return 0;
