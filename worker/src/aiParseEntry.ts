...
export type AiEntryParseResult = {
  parsed: ParsedSignal
...
function aiTimeoutMs(): number {
  return Math.max(500, Math.min(15_000, Number(process.env.AI_ENTRY_PARSE_TIMEOUT_MS ?? 4000)))
}
...
function toParseResult(
  ...
  return { parsed: parsed as ParseChannelMessageResult['parsed'], status, skip_reason }
}
...
export async function aiParseEntry(
  ...
  const baseContext = await buildAiModificationContext(supabase, {
    userId: args.userId,
