...
import { isAutoBeTriggerMet } from '../autoManagement'
...
async function main() {
  ...
  for (const b of brokers ?? []) {
    ...
    const cfgs = (b.channel_trading_configs ?? {}) as Record<string, { manual_settings?: Record<string, unknown> }>
    for (const cfg of Object.values(cfgs)) {
      const cms = cfg?.manual_settings ?? {}
      if (cms.use_predefined_sl_pips === true) channelPreSl++
      if (cms.use_predefined_tp_pips === true) channelPreTp++
      const mode = String(cms.move_sl_to_entry_after_mode ?? 'none')
      if (mode !== 'none' && mode !== '') channelAutoBe++
    }
  ...
  for (const trade of autoTrades ?? []) {
    ...
    try {
      ...
      const pip = signalPipPrice(trade.symbol)
      const mode = String(trade.auto_be_mode) as 'pips' | 'rr' | 'money' | 'tp_hit'
      const triggerMet = isAutoBeTriggerMet({
        mode,
      ...
      const profitPips = isBuy
        ...
        : (entry - q.ask) / pip
      console.log(
        `  ${trade.symbol} ${trade.direction} entry=${entry} sl=${trade.sl} mode=${mode}`
  ...
  const sampleBroker = brokers?.find(b => Object.keys((b.channel_trading_configs ?? {}) as object).length > 0)
  if (sampleBroker) {
    const cfgs = (sampleBroker.channel_trading_configs ?? {}) as Record<string, { manual_settings?: Record<string, unknown> }>
    const first = Object.entries(cfgs).find(([, c]) =>
      c.manual_settings?.use_predefined_sl_pips === true || c.manual_settings?.use_predefined_tp_pips === true,
    )
    if (first) {
      const [, cfg] = first
      const manual = cfg.manual_settings ?? {}
      const derived = deriveManualStopsWithClamp({
        parsed: { action: 'buy', symbol: 'XAUUSD', sl: 4490, tp: [4510], entry_price: null } as never,
