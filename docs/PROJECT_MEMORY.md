...
- **Deploy state:** Code written on `migration`, **not committed / not deployed**. FXSocket quote path works without edge redeploy; MTAPI quotes need `mtapi-broker` edge redeploy after commit (until then MTAPI Current shows `—` with `Unknown action: quote`; FxSocket works).
...
- **Follow-ups:** commit when user asks; redeploy `mtapi-broker` to migration Supabase; smoke-test Current price on an open trade.
...
- **Deploy state:** Fix written on `migration`, **not committed / not deployed**. Until Railway Trade rebuilds, the failed-tick loop and notification retention pressure continue on migration.
- **Related (not fixed here):** retention priority list in `prune_all_trade_execution_logs` should eventually include notification-eligible actions (`order_send`, `partial_tp_fired`, `mgmt_*`, `merge_modify_summary`) so monitor-noise floods cannot evict user-visible trade logs — see `docs/scratchpad-notifications-migration-2026-09-24.md`.
