...
### 2026-09-25 — Notification retention hardened (three tiers) + "Live price" label

- **Plain English:** Two things. First, the notification problem is fully handled: the failed-log flood stopped by itself once yesterday's reconcile fix deployed, the feed is alive again, and we closed the door on it happening again. The retention job that trims old log rows used to treat only internal system rows as "important," so a flood of those could push out the trade updates users actually see — retention now protects user-visible trade actions first, internal diagnostics second, and prunes everything else first. Applied to both the staging and production databases. Second, the trade detail popup's price line was renamed from "Current" to **"Live price"** in every language, and the number is now slightly bolder.
- **Root cause (technical):** `prune_all_trade_execution_logs` kept the newest 500 `trade_execution_logs` rows per user with priority = only the 22 internal pipeline/queue actions (`basket_reconcile_tick`, `queue_*`, `dispatch_*`, …). Displayable actions (`order_send`, `partial_tp_fired`, `mgmt_*`, `merge_modify_summary`, …) ranked tier 0, so any sustained internal-noise flood evicted them first — the mechanism behind the empty Notifications panel (documented in the 2026-09-24 SL-drift entry).
- **Fix (files):**
  - `supabase/migrations/20260925070000_retention_priority_trade_actions.sql` — `CREATE OR REPLACE public.prune_all_trade_execution_logs` with a three-tier `CASE` (DESC): **tier 2** = displayable actions (retry-eligible subset from `src/lib/tradeActivities.ts`, `PIPELINE_ACTIONS`, plus `broker_manual_stop_override_reverted`, `plan_fallback`, `signal_entry_pending_placed/failed/cancelled`, `virtual_pending_failed`, `range_basket_tp_rebalance`, `user_force_close`, `dispatch_claim_error`, and `LIKE 'mgmt\_%'`); **tier 1** = the previous 22 internal actions; **tier 0** = everything else. `p_keep < 500` floor kept (dead `<1` guard removed); `SECURITY DEFINER` + `search_path` + COMMENT preserved; a tier-2 flood can only displace internal diagnostics, never user-visible rows.
  - Applied twice (v1, then review-driven v2) via Management API `POST /v1/projects/{ref}/database/migrations` to **both** `supmsgcubipmmowrzoub` (migration) and `sxkpcovbyaficvtkpsdo` (production); verified on both via `pg_proc.prosrc` (nine extra actions + tier 2 + escaped wildcard present). The read path is unchanged — the feed still reads the newest 500 rows.
  - i18n: `colCurrent` value renamed `'Live price'` (en), `'Precio en vivo'` (es), `'Prix en direct'` (fr), `'السعر المباشر'` (ar), `'ライブ価格'` (ja), `'Live prijs'` (nl), `'Cena na żywo'` (pl), `'Актуальная цена'` (ru), `'Livepris'` (sv) — key name unchanged.
  - `src/components/trades/TradeDetailModal.tsx` — live price span `font-medium` → `font-semibold`.
- **Tests/verification:** `npx tsc -b` clean; i18n types test 5/5; ESLint on all changed files = only the 2 pre-existing TradeDetailModal errors (`set-state-in-effect` @66, `preserve-manual-memoization` @165); SQL verified against both databases after each apply.
- **Reviews (mandatory):** code-tester PASS; code-review PASS_WITH_NOTES → MEDIUMs applied (nine missing displayable actions; two-tier → three-tier ranking) + LOWs applied (dead `p_keep` guard removed, `mgmt\_%` underscore escaped, ru label retranslated to `Актуальная цена`).
- **Investigation notes:** the flood's last failed `basket_reconcile_tick` row was 2026-09-24 09:07 UTC (pre-fix); affected user's feed had 70 displayable rows in the newest-500 window, latest 05:40 UTC. New Supabase token (rotated this session) stored in `~/.supabase/access-token` — old token had expired and blocked the DB checks.
- **Deploy state:** DB function replaced on migration **and** production (via Management API); commits `28d65f47` (retention migration) + `38581a63` (label/bold) pushed to both remotes (branch tip `38581a63`); Railway Trade + Listener deployed SUCCESS at 07:48 UTC 2026-09-25; `force-close-trades` edge redeployed to migration Supabase in the same push cycle (close-trade feature, see previous entry).
- **Follow-ups (non-blocking):** re-check the feed window only if a future "no recent activity" complaint arrives (tier-2 rows can now only squeeze internal diagnostics); optional smoke-test of the Live price line on an open trade.

### 2026-09-24 — "Close this trade" button in the Edit SL/TP modal (with 10-second result summary)

- **Plain English:** Until now, the only way to close a signal's positions from the app was the broker-level or channel-level force close. The Edit SL/TP modal could change stop loss and take profit, but not exit the trade. We added a **Close this trade** button: it asks for confirmation, closes every open position of that one signal across all of the user's brokers, and then shows a short summary (how many closed, how many failed, how many queued entry orders were removed) that automatically closes itself after 10 seconds. The page behind the modal shows the same summary as a banner and refreshes its rows. If nothing was actually closed, the user gets a clear error and the dialog stays open — a green "success" is never shown for a failed close.
- **Root cause (technical):** Feature gap. The existing `force-close-trades` API is broker+channel scope only (`useForceCloseSignalTrades`); there was no signal-scoped path. The signal-override flow (`applySignalOverride`) already proved the multi-broker pattern, so the close path mirrors it end to end.
- **Design:** signal ownership is verified twice (edge checks `signals.user_id`, worker re-checks with `user_id` filters on `signals`/`trades`/`range_pending_legs`/`broker_accounts`); queued layering legs (`range_pending_legs`) are swept on every broker that holds this signal — even brokers without a live session or without a materialized trade — so the plan cannot reopen after the close; orphan legs (broker deleted, `trades.broker_account_id` SET NULL) are counted as failed instead of vanishing.
- **Fix (files):**
  - Worker: `worker/src/forceCloseSignalTrades.ts` — new `forceCloseSignalById` (discovery → per-broker `closeWithVerification` → DB status update → `clearChannelActiveTradeParamsWhenFlat` → `deleteRangePendingLegsForBasket` → `insertForceCloseLog` with `scope: 'signal'`); `insertForceCloseLog` scope union += `'signal'`; returns `broker_not_connected` (ok:false, legs counted) when no broker could be reached, `no_open_trades` (ok:true) only when genuinely nothing exists; all five discovery queries (`signals`, `trades` union, `range_pending_legs` union, `broker_accounts`, leg load) now surface DB errors as `reason:'close_failed'` instead of being swallowed into "nothing to close".
  - Worker: `worker/src/managementScope.ts` — new `loadTradesForBasketAnchorChecked` ({rows, error}); `loadTradesForBasketAnchor` delegates to it with byte-identical semantics for its four existing callers.
  - Worker: `worker/src/httpServer.ts` — `/internal/force-close-trades` accepts optional `signal_id` (requires `user_id` + (`broker_account_id` OR `signal_id`)); shard check kept.
  - Edge: `supabase/functions/force-close-trades/index.ts` — `signal_id` branch (ownership check → worker `{user_id, signal_id}`); raw DB/worker error text is logged server-side and replaced with generic `"Force close failed"` (404 existence strings kept).
  - Frontend: `src/lib/forceCloseTradesApi.ts` — discriminated-union request (`signal_id` XOR `broker_account_id`), 120s `AbortSignal.timeout`, 200-body shape check (numeric `closed` required), timeout-specific error message.
  - Frontend: `src/components/dashboard/EditSignalOverrideModal.tsx` — state machine `edit → confirmClose → result`; Update → green/amber summary + 10s countdown auto-close; Close this trade → amber confirmation view → result summary; guard `closed === 0 && (failed > 0 || virtual === 0)` stays on the confirm view with a specific error; error path still notifies the parent when queued legs were swept so the page refreshes; partial results render amber, not green; `catch` whitelists only `'Not signed in'`/`'Signal not found'` (the latter localized to `closeSignalGone`), everything else → generic retry text; double-submit guarded by `closeInFlightRef`, Escape/backdrop disabled while busy.
  - Frontend: `src/pages/dashboard/SignalHistoryPage.tsx` — `handleClosedFromModal` mirrors the modal's numbers exactly (partial warning when `failed > 0`, queued-only success when nothing was open, `0 of N` warning when closes failed but the queue was swept) and runs `refreshAfterOverrideSave`.
  - i18n: 16 new `signalHistoryPage` keys (`closeTrade`, `closeConfirmTitle/Body`, `confirmClose`, `cancelAction`, `closing`, `closeSuccess`, `closePartial`, `closeFailed`, `closeNoOpenTrades`, `closeNotConnected`, `closeRetry`, `closeQueuedRemoved`, `closeSignalGone`, `autoCloseIn`, `doneTitle`) in `types.ts` + `en/es/fr` + `signalHistory/ar` (es/fr strings are English, matching the pre-existing state of that block).
  - Tests: `worker/src/forceCloseSignalTrades.test.ts` — 12 tests: channel-path guards, signal ownership (`user_id` filter), `broker_not_connected` with counted legs, skipped-broker queued-leg sweep (union→cancel→delete call sequence), and three DB-error tests (signal lookup, broker discovery, leg load → `close_failed`, mutation-verified).
- **Tests/verification:** worker `tsc` clean; full worker suite **1993/1993**; target file 12/12; `httpServer.authErrors` 6/6; `managementScope`/`channelStopApply`/`basketLegSync`/`replyScope` 43/43; `npx tsc -b` clean; `npx deno check` on the edge clean; ESLint on changed files = only the 3 known pre-existing errors (modal `react-refresh` @89, `SignalHistoryPage` refs @183 + set-state @263); i18n types test 5/5.
- **Reviews (mandatory):** round 1 code-tester PASS / code-review FAIL (HIGH: broker select missing `mtapi_session_id`; HIGH: `ok:false` shown as green; MEDIUMs: queued legs survive, skipped-broker legs uncounted, reason not surfaced) → all fixed. Round 2 PASS / FAIL (HIGH: modal guard missed `failed>0` case; MEDIUMs: banner contradicted modal, skipped-broker queue not swept, raw internal errors in catch) → all fixed. Round 3 PASS / PASS_WITH_NOTES (MEDIUMs: DB errors swallowed into "nothing to close"; raw error text still reachable) → fixed + LOWs (error-path parent notify, partial amber tone, 120s timeout, ownership test breadth). Round 4 PASS / PASS_WITH_NOTES (MEDIUMs: banner wrong for `closed=0,failed>0`; leg-load error swallowed) → fixed + LOWs (edge stops echoing raw text, shape check, localized `Signal not found`, three DB-error tests). Round 5 **code-tester PASS / code-review PASS_WITH_NOTES** (LOWs only; scores Security A, Correctness A-, Quality A, Readiness A).
- **Deploy state:** Committed as `c4835f88` and pushed to both `origin/migration` and `upstream migration` (branch tip `38581a63`); Railway Trade + Listener deployed SUCCESS at 07:48 UTC 2026-09-25; `force-close-trades` edge function redeployed to migration Supabase — the close button is live.
- **Follow-ups (round-5 LOWs, not blocking):** `deleteRangePendingLegsForBasket` still swallows DB errors (shared helper; a failed sweep with no trades can read as `no_open_trades`); modal `catch` path does not trigger a parent refresh (stale queued-leg rows after a timeout); `markDone` ignores the trades status-update error (pre-existing pattern shared with `forceCloseChannelOnBroker`); es/fr translations for the new keys; optional shared `formatCloseSummary` helper to deduplicate modal/banner branching.

### 2026-09-24 — Live current price in Trade detail modal

- **Plain English:** The trade detail popup only showed entry, stop loss, and take profit — the market price moving right now was missing. Open trades now show a "Current" line that refreshes every 10 seconds with the price that would close the position (bid for buys, ask for sells), plus a bid/ask line. It works for both FxSocket and MTAPI brokers. If the price cannot be fetched, the field shows an em dash instead of a stale number.
- **Root cause (technical):** Feature gap — no quote path existed in the detail modal. FxSocket already had a `quote` action; MTAPI needed `GetQuote` support on the edge function.
- **Fix (files):**
  - Edge: `supabase/functions/_shared/mtapiClient.ts` — `getQuote(sessionId, symbol, platform)` calls `GET /GetQuote`, normalizes bid/ask, rejects invalid prices with `MtapiApiError("GetQuote returned invalid prices", 502, "INVALID_RESPONSE")`.
  - Edge: `supabase/functions/mtapi-broker/index.ts` — new `quote` action in the shared `opened_orders`/`quote` block; returns `{ ok: true, quote: {} }` when no session; `symbol` defaults `EURUSD`.
  - Frontend: `src/lib/fxsocketBroker.ts` — `quote(accountId, symbol, provider?)` routes `edgeFn: 'mtapi-broker'` when `provider==='mtapi'`, timeout 15s.
  - Frontend: `src/lib/tradeLiveQuote.ts` (new) — pure helpers: `normalizeTradeQuote`, `shouldPollTradeQuote` (open + broker_id + symbol), `liveExitPrice` (buy→bid, sell→ask, fallback), `formatBidAskLine`.
  - Frontend: `src/hooks/useTradeLiveQuote.ts` (new) — 10s poll (`LIVE_QUOTE_POLL_MS`), immediate first fetch, pauses when tab hidden, single-flight, provider cached in module `Map` after one `broker_accounts.provider` lookup, stops when trade not pollable.
  - Frontend: `src/components/trades/TradeDetailModal.tsx` — `colCurrent` row after TP for open trades: exit-side price, bid/ask line, spinner while loading, `—` on error.
  - i18n: `colCurrent` added to `types.ts`, `en.ts`/`es.ts`/`fr.ts`, and trading locales `ar/pl/ru/nl/ja/sv`.
  - Tests: `src/lib/tradeLiveQuote.test.ts` (vitest, 8); `supabase/functions/_shared/mtapiClient.test.ts` (2 new Deno tests for getQuote).
- **Tests/verification:** `npx tsc -b` clean; vitest `tradeLiveQuote` 8/8; Deno `mtapiClient.test.ts` 12/12; ESLint clean on the new/changed files (TradeDetailModal `set-state-in-effect` and `mtapi-broker` `_pw`/`_sid` are pre-existing). **Reviews:** code-tester PASS; code-review round 1 PASS_WITH_NOTES → MEDIUMs fixed (hook setState/stale price, attempts reset) → round 2 PASS_WITH_NOTES (residual LOW only) → final key-aware setState spot-check PASS.
- **Deploy state:** Committed as `bd93a9cd` and pushed to both `origin/migration` and `upstream migration`; Railway Trade + Listener deployed SUCCESS at 18:25 UTC 2026-09-24; `mtapi-broker` edge redeployed to migration Supabase — FxSocket and MTAPI Current prices both live.
- **Review notes (applied):** hook no longer `setState`s synchronously in the effect (`!poll` returns `IDLE` at render; first `setState` after `await resolveQuoteProvider` and only spreads prior state when the trade key is unchanged — otherwise clears quote/price so trade A's price cannot show under trade B); failed fetch clears `price`/`quote` so the UI shows `—` not a stale number (matches this entry); spinner shows on every in-flight load/refresh; visibility timer is nulled when fired; quote state is keyed to `tradeId|symbol`. Residual LOW: reopening the same trade can flash the previous price for one fetch (self-heals; modal stays mounted with `trade={null}` between opens). Residual ops: orphaned `basket-sl-tp-sweep` edge-function slug still deployed on migration Supabase (no cron row).
- **Follow-ups:** smoke-test the Live price line on an open trade (FXSocket and MTAPI); the line was later renamed "Live price" and made slightly bolder — see the 2026-09-25 entry.

### 2026-09-24 — V1 basket reconcile SL-drift loop (ghost legs + empty OpenedOrders)

- **Plain English:** On migration, one user's account produced hundreds of failed "stop loss still drifted" log rows every hour even though the positions were already gone on the broker. The reconcile worker kept retrying an impossible fix instead of recognizing the trades had closed and cleaning them up. An empty broker response (brief disconnect) was also treated as "nothing is open," which made the false failures worse. We fixed the worker so an empty response is inconclusive (retry later), and when every open trade is missing from a valid broker list it closes the stale database rows and stops. Until this deploys, the failed-log flood continues and also pushes out the user's real trade notifications (retention keeps the noise, drops the signal).
- **Root cause (technical):** MTAPI is forced onto the V1 `BasketSlTpReconcileMonitor` (`resolveExecutionEngine`: `provider==='mtapi'` → `'v1'`). When every open DB leg was `skipped_not_on_broker` (ticket absent from a non-empty OpenedOrders snapshot), the post-pass `basketLegsOutOfSyncOnBroker` still compared DB SL vs fresh targets and set `brokerStillDrift = true` → log `failed` + requeue. Unlike `slTpRefresh.ts` (`allLegsGhostOnBroker` → `closeStaleOpenTrades`) and `openTradeReconcile.ts` (non-empty snapshot required), V1 never closed ghosts. Preflight `usePreflight = size >= 0` is always true, so an empty OpenedOrders map was treated as "nothing open" and every leg was skipped. Evidence: 460 failed `basket_reconcile_tick` rows 04:07–09:07 UTC for user `20733d28` (signals `1fcca2ff` 249 fails / trade `d0444e40` ticket `3274402356`; `5260104c` 211 fails / trade `c94b229b` ticket `3270049337`); staging same user 0 fails. Side effect: those 457+ rows hit the 500-row retention cap; `order_send`/`partial_tp_fired` are **not** on the retention priority list so they were pruned first — Notifications panel showed "No recent trade activity yet."
- **Fix (files):**
  - `worker/src/basketSlTpReconcileMonitor.ts` — export `isAllLegsGhostOnBroker` + `isEmptyOpenedOrdersInconclusive`; empty OpenedOrders → `releaseJob` (inconclusive) instead of failing drift; all-legs-ghost on non-empty snapshot → `closeStaleOpenTrades` + success log (`ghost_closed: true`) + `markBasketReconcileDone` + flip source signal `parsed → executed` (parity with the normal success path); failure message now includes `'; N not on broker'`.
  - `worker/src/basketSlTpReconcile.ts` — `upsertBasketReconcileJob` now sets `attempts: 0` on re-enqueue so a job that burned its budget on empty-snapshot releases can still run the ghost-close path after the next sweep.
  - `worker/src/basketLegSync.test.ts` — new "ghost / empty snapshot guards" suite (4 tests).
  - Scratchpad: `docs/scratchpad-zejaymes-migration-sl-drift-2026-09-24.md`.
- **Ops note:** migration Supabase still has the orphaned edge function `basket-sl-tp-sweep` (no `cron.job` row — only `signal-reconcile-sweep` remains for sweeps). Safe to delete the function when convenient so a manual invoke cannot bypass this fix.
- **Tests/verification:** worker `tsc` clean; `basketLegSync.test.ts` ghost/stale-claim 4/4; `basketSlTpReconcile.test.ts` 20/20; ESLint clean on the monitor + upsert files. **Reviews:** code-tester PASS; code-review round 1 PASS_WITH_NOTES → `attempts: 0` + ghost-path `parsed→executed` applied → round 2 PASS_WITH_NOTES (no CRITICAL/HIGH; residual LOWs only) → final spot-check PASS.
- **Deploy state:** Committed as `5951762c` and pushed to both `origin/migration` and `upstream migration`; Railway Trade + Listener deployed SUCCESS at 18:25 UTC 2026-09-24 — the failed-tick loop and ghost-close path are live on migration.
- **Related (fixed 2026-09-25):** the retention priority gap noted here (displayable actions pruned first under noise floods) was closed by `supabase/migrations/20260925070000_retention_priority_trade_actions.sql` — three-tier ranking applied to migration and production; see the 2026-09-25 entry.
- **Follow-ups:** commit + deploy monitor fix when user asks; optional retention-priority migration; re-check failed `basket_reconcile_tick` count after deploy; ghost trade `c94b229b` (ticket `3270049337`) already closed in DB at 09:51:27 by a later close-signal path (no longer needs manual close).

### 2026-09-24 — Replace MTAPI IP allowlist with X-Internal-Token + WireGuard scripts

- **Plain English:** A security check that only allowed certain server IP addresses to talk to our MTAPI bridge started blocking our own app. Railway (where our worker runs) changed its outgoing IP address, and Supabase edge functions use Amazon's IP addresses — both were therefore rejected with "IP not allowed." A test trade (XAUUSD sell) failed for this reason. We are replacing the IP list with a shared secret header (`X-Internal-Token`) that our edge and worker already send when configured, and we prepared optional WireGuard scripts so the worker can reach the bridge over a private encrypted tunnel instead of the public internet.
- **Root cause (technical):** nginx `geo $allowed_ip` on the VPS listed fixed IPs (`152.55.177.10`, etc.). Railway egress rotated to `152.55.177.237` (108× 403, UA `node`); Supabase edge egress is AWS (`18.157.158.148` / `18.192.21.197`, UA `Deno/…SupabaseEdgeRuntime`). IP allowlists rot and break live callers.
- **Fix (files):**
  - Edge: `supabase/functions/_shared/mtapiClient.ts` — send `X-Internal-Token` from `MTAPI_INTERNAL_TOKEN`; tests in `mtapiClient.test.ts`.
  - Worker: `worker/src/mtapiProvider.ts` — same header; tests in `mtapiProvider.test.ts`.
  - nginx (local gitignored copies): `docs/nginx-setup/nginx.conf` — `map $http_x_internal_token $internal_ok`; `docs/nginx-setup/mtapi-tscopier.conf` — check `$internal_ok`, disable `geo $allowed_ip`, update `@forbidden` message; `setup.sh` generates `/root/.mtapi_internal_token`.
  - WireGuard (option C): `deploy/wireguard/` — `vps-setup.sh`, `peer-worker.conf.example`, `mtapi-tscopier-tunnel.conf`, `README.md`.
  - Docs: `.env.example`, `worker/.env.example`, `docs/mtapi-credentials.md`, `docs/nginx-setup/README.md`.
- **Tests/verification:** edge Deno `mtapiClient.test.ts` 10/10 (incl. token present/absent/whitespace); worker `mtapiProvider.test.ts` 17/17 (same); `deno check mtapiClient.ts` clean; ESLint on the 4 changed TS files 0 errors; shell `bash -n` OK. Full worker `tsc` / full-repo lint skipped (WSL OOM). nginx auth matrix + empty-token fail-open verified against rendered configs; `setup.sh` hard-fails on empty/short token, renders to a temp file, greps both `"" 1` and empty-Bearer, `chmod 600`, and restores a backup if `nginx -t` fails after replace. WireGuard wg0-down claim verified empirically on nginx 1.24: production wildcard `listen 9443` + tunnel `listen 10.66.0.1:9443` starts with the tunnel IP absent (public serves); explicit-IP public listen would fail — docs state that caveat. Certbot renewal fix: permanent ACME-only `listen 80` block in `mtapi-tscopier.conf`, `ufw allow 80/tcp` kept (not deleted), temp certbot conf is challenge-only (`return 444` elsewhere) and is skipped on re-run when the live conf matches `listen 9443`. `setup.sh` also `ufw allow 9443/tcp`. **Final verdict: code-review PASS / PASS_WITH_NOTES (no CRITICAL/HIGH); code-tester PASS / PASS_WITH_NOTES (static green; runtime deno/tsx/bash -n blocked in subagent sandbox but run green in parent).**
- **Deploy state (cutover complete 2026-09-24):**
  - Edge function `mtapi-broker` deployed to migration Supabase with `X-Internal-Token` support.
  - Worker commits pushed to `origin/migration` and `upstream/migration`: `54e62948` (header), `65bfba93` (WireGuard entrypoint), `06ce2fd7` (WireGuard best-effort fix). Railway Trade (`migration` env) rebuilt and running `06ce2fd7`.
  - `MTAPI_INTERNAL_TOKEN` **set** on migration Supabase Edge secrets (confirmed via `secrets list`) and on Railway Trade vars.
  - VPS nginx: `nginx.conf` maps for `$internal_ok` / `$auth_ok` **rendered with real values** (initial scp left raw `${…}` placeholders — fixed with `sed` + reload). `mtapi-tscopier.conf` applied: token check live, `geo $allowed_ip` disabled, ACME-only `listen 80` kept. Verified: `/health` **200**, no-token **403**, token+Bearer **404** (auth passed).
  - **WireGuard (option C) blocked on Railway:** containers lack `NET_ADMIN`; `wg-quick`/`wireguard-go` both fail with `Operation not permitted`. First entrypoint used `set -e` and crash-looped the trade worker before `node` started; fixed in `06ce2fd7` to best-effort (log + continue on public HTTPS). VPS `wg0` server and tunnel nginx block remain installed and healthy from the VPS side; peer never joins from Railway. Option D (header token) alone carries the fix.
- **Follow-ups:** re-run Trades XAUUSD smoke test (post-cutover, not yet recorded); decide whether to keep or drop WireGuard scripts/`WG0_CONF` on Railway; password rotation still open (separate incident); optional `upstream` push of remaining uncommitted docs/incidents.

### 2026-09-23 — Broker password visible in MTAPI Docker logs (security incident)

- **Plain English:** While checking the MTAPI bridge logs on the Contabo server, a broker account password appeared in clear text. TScopier application code did not write it. The third-party MTAPI program prints the account number and password whenever it logs a warning (for example when a symbol name is wrong). Anyone with root on the server, or anyone who pastes those logs into a ticket or chat, can read the password. The safe response is to change the broker password immediately, wipe the old log files, and keep the MTAPI container port closed to the public internet. Our application already stores passwords encrypted and strips them from API responses — that path is not the leak. Separately, the same log dump showed automated scanners hitting port 8080 on the public IP.
- **Root cause (technical):** `timurila/mt5rest` logs connection tuples `(login, password, ip, …)` on WRN/ERR paths (e.g. `SymbolParams` → `Symbol not found`). Docker `json-file` retains plaintext under `/var/lib/docker/containers/*/*-json.log`. Not caused by edge/worker logging: AES-256-GCM at rest, `stripSecrets` on responses, Sentry redacts `broker_password`; nginx already uses redacted access log (no query string). Related known limitation: worker `mtapiProvider` warns that `ConnectEx` puts password in the URL query string — access logs already drop query strings.
- **Containment (user/ops on VPS — agent cannot SSH):** (1) **Done:** redactor `install.sh` run on VPS — container recreated with `redact.awk` filter, historical json logs for that container truncated, MTAPI restarted healthy (session check still works); (2) **Done:** public `:8080` closed by rebinding the container to `-p 127.0.0.1:8080:80` (2026-09-23). Note: `ufw deny 8080` alone did **not** work — Docker published ports bypass UFW (external probe had still returned `Healthy`); after the localhost bind, external `:8080` times out and `:9443` stays Healthy; (3) **Open:** rotate password for login `476205231` (Exness-MT5Trial9) and reconnect in TScopier; (4) **Open after rotation:** verify old password absent from new logs / any WRN shows `[REDACTED]`.
- **Log redactor (code, in-repo):** `deploy/mtapi-log-redactor/` wraps the container entrypoint so stdout/stderr pass through `redact.awk` before Docker persists them. Redacts connection tuples `('login' 'PASSWORD' …)`, `password=` query values, JSON `"password"`, and config-style `password: '…'`. Local tests: `bash deploy/mtapi-log-redactor/test.sh` (gawk + mawk). **Installed on VPS 2026-09-23** (`/opt/mtapi-redactor`; original entrypoint `dotnet mt5rest.dll`).
- **Secrets reference:** `docs/mtapi-credentials.md` — `BROKER_CREDENTIALS_ENCRYPTION_KEY` (AES-256-GCM at rest; missing on Edge → plaintext fallback), `MTAPI_BASE_URL` (bridge URL for edge + worker), `MTAPI_API_KEY` / `MTAPI_PROXY_KEY` (Bearer key nginx checks).
- **Incident docs:** `docs/incidents/incident-2026-09-23-mtapi-docker-password-logs.md/.html/.pdf`; scratchpad `docs/scratchpads/scratchpad-mtapi-password-in-logs-2026-09-23.md`.
- **Deploy state:** Redactor **installed on VPS**; public `:8080` **closed** (localhost bind, 2026-09-23; UFW alone was insufficient because Docker bypasses it). Status remains **Open** for password rotation and post-rotation log verify. Redactor package still uncommitted on `migration`.
- **Edge secrets (2026-09-23):** `MTAPI_BASE_URL=https://mtapi.tscopier.ai:9443` + `MTAPI_API_KEY` **set** on migration Supabase (`supmsgcubipmmowrzoub`) via dashboard; `secrets list` shows both digests. Railway Trade already had both vars (with `:9443`).
- **Follow-ups:** user rotates broker password for `476205231` and reconnects; confirm new logs have no plaintext password; long-term MTAPI log verbosity / upstream redaction; ~~re-enable IP allowlist~~ **superseded 2026-09-24** — IP allowlist permanently replaced by `X-Internal-Token` (see 2026-09-24 entry); same image will host production sessions — demo leak is a process warning, not “demo-only”.

### 2026-09-23 — Trades page reads MTAPI history (provider fan-out edge + UI)

- **Plain English:** After connecting an MTAPI broker, trades executed through it did not show up on the Account Trades page, sidebar open-trade indicator, or performance charts. Those screens only talked to the FxSocket edge function, so they never asked the MTAPI bridge for history. We added an MTAPI trades path on the edge, and the frontend now asks both providers at once and merges the results. Wrong-provider errors are ignored as long as at least one provider returns data. Balance/summary refresh for MTAPI accounts now really calls the bridge instead of only returning the cached database row.
- **Root cause (technical):** UI fetch paths (`useTradesData`, `useHasOpenTrades`, `brokerTradeHistory`/`fetchBrokerMtTrades`, performance payload) called `fxsocketBroker.trades()` only. `mtapi-broker` edge had no `trades` / history-hydration actions. Dashboard/performance gates used `isFxsocketLinkedBroker`, skipping MTAPI-only users.
- **Fix (files):**
  - **New** `supabase/functions/_shared/mtapiClient.ts` — Deno edge MTAPI HTTP client (`OpenedOrders`, `OrderHistory`, `HistoryPositions`, `ClosedOrders`, `AccountSummary`, Bearer auth, `?id=` session, list unwrapping, `MtapiApiError`, `normalizeMtapiAccountSummary`).
  - `supabase/functions/_shared/fxsocketTrades.ts` — shared pipeline now takes `MtHistorySource` so one implementation serves both providers.
  - `supabase/functions/mtapi-broker/index.ts` — added `trades`, `opened_orders`, `order_history`, `position_history`; real `refresh_summary` (AccountSummary + persist balance/equity/baseline with non-fatal fallback); fixed `bad(403, limitDenied)` to parse the `Response` body like `fxsocket-broker`.
  - `src/lib/fxsocketBroker.ts` — `provider` on history methods; `fetchTradesAcrossProviders()` fan-out (`Promise.allSettled`, merge, wrong-provider filter, throw only if all fail or empty+real failure).
  - Callers: `useTradesData.ts`, `useHasOpenTrades.ts`, `brokerTradeHistory.ts`, `mtTradeTimestamps.ts` (hydration), `usePerformanceData.ts`, `DashboardPage.tsx` (client gates → `hasLinkedBrokerForUi` / `countLinkedBrokerSessionsForUi`; WS/live-stream still FxSocket-only on purpose).
  - **Tests:** `src/lib/fxsocketBroker.trades.test.ts` (vitest, 7); `supabase/functions/_shared/mtapiClient.test.ts` (Deno, 7).
- **Tests/verification:** `tsc -p tsconfig.app.json --noEmit` clean; vitest 7/7 + deno 7/7; targeted node brokerLink/mtTradeTimestamps 24/24; full frontend suite: 365 pass / 1 pre-existing fail (`channelWorkerLogMessage`); lint on changed files only pre-existing `set-state-in-effect` (baseline same); DashboardPage problem count unchanged vs baseline (76).
- **Deploy state:** Reviews pass (code-tester PASS, code-review PASS_WITH_NOTES). `mtapi-broker` (v8) and `fxsocket-broker` (v92) deployed to migration Supabase (`supmsgcubipmmowrzoub`). Railway migration already has `MTAPI_BASE_URL` + `MTAPI_API_KEY` + `BROKER_CREDENTIALS_ENCRYPTION_KEY` on Listener and Trade (values not readable via API). **Edge secrets now set (2026-09-23, dashboard):** `MTAPI_BASE_URL=https://mtapi.tscopier.ai:9443` + `MTAPI_API_KEY` — confirmed via `secrets list`. Product code still **uncommitted** on `migration`; origin push allowed, upstream push needs explicit user permission.
- **Follow-ups:** verify Trades page shows ticket **3269773910** for user `20733d28` after deploy; MTAPI WS/live-stream parity still open; password-in-logs incident (separate entry above).

### 2026-09-23 — MTAPI connect pipeline verified live; nginx config corruption incident

- **Plain English:** We finished wiring the MTAPI bridge into the product so a user can add a broker account and have it connect through our own MTAPI server instead of FxSocket. A new edge function (`mtapi-broker`) handles connecting, reconnecting, refreshing, status, and deleting MTAPI accounts; the worker provisions the account, establishes a session, and keeps balance and equity up to date. We verified end to end that an account connects and the worker syncs it. While applying a security improvement to the web server (redacting broker passwords from access logs), a shell command mangled the nginx config on the VPS and briefly broke the config test; we rebuilt the file cleanly from the repo version and confirmed the endpoint is healthy again. We also confirmed the access logs no longer contain passwords.
- **What changed (technical):**
  - New `supabase/functions/mtapi-broker/index.ts` — actions `connect`, `reconnect`, `refresh_summary`, `broker_status`, `delete`. Uses the service role to write credential columns that the client is blocked from writing by the `broker_accounts_guard_mtapi_credentials` trigger. Strips secrets from responses.
  - `supabase/functions/fxsocket-broker/index.ts` — reverted to FxSocket-only; added `stripSecrets` to the connect/reconnect/refresh_summary/list responses; removed the `.neq("fxsocket_account_id","")` list filter so MTAPI accounts are not hidden from the bulk-connect dedupe.
  - `supabase/functions/_shared/brokerCredentialsCrypto.ts` — `readEnv` fixed to use `env.get()` (works with both `Deno.env` and plain records).
  - `worker/src/mtapiSessionManager.ts` — `provisionNewAccounts` decrypts the stored password, calls `ConnectEx`, and persists the session; `syncAccountState` writes balance, equity, currency, `last_synced_at`, and `performance_baseline` on each sweep and after provisioning.
  - `worker/src/mtapiProvider.ts` — sends `Authorization: Bearer` (nginx expects Bearer, not `X-MTAPI-Key`).
  - Frontend — `fxsocketBroker` routes every broker action to the correct edge function via an `edgeFn`/`provider` parameter; `BrokerStatusModal` is provider-aware; the `BrokerAccount` type gained the MTAPI fields and dropped `broker_password_encrypted`.
- **Root cause (technical) — nginx incident:** A `sed -i '/log_format main /a\ ...'` append command was pasted through the terminal. The em-dash in the inserted comment was corrupted by the terminal encoding, and the multi-line `log_format main` statement (nginx's default is a 3-line directive) was split so the new line landed in the middle of it. A later cleanup step deleted one of the original continuation lines, leaving an orphaned fragment that nginx reported as `unknown directive`. A second `sed` using `a\` matched every line and appended `ccess_log ...` after each one, corrupting `mtapi-tscopier.conf` beyond repair (the backup had been taken after the damage). Fix: rebuilt the logging block in `nginx.conf` with a Python `re.sub`, and rewrote `mtapi-tscopier.conf` from the repo version via a quoted heredoc. `nginx -t` now passes and `/health` returns `Healthy`.
- **Security:** Added a redacted nginx log format (`mtapi_safe`) that logs `$request_method $uri` without the query string, so broker passwords passed as query parameters never reach disk. Verified: `/ConnectByToken` and `/AccountSummary` appear in `mtapi-access.log` with no `?args`. **IP allowlist ENABLED 2026-09-24** after discovering real worker egress from the access log (not guessed Railway ranges): `152.55.177.10` (trade worker, UA `node`), `13.57.96.134` / `54.219.72.7` (session sync), `162.220.232.0/24`, plus Railway published ranges. `if ($allowed_ip = 0) { return 403; }` was active. **Note:** IP allowlist later replaced the same day by `X-Internal-Token` auth (see 2026-09-24 entry); `/health` remains open; API-key auth, TLS, rate limiting, and scanner-path blocking remain active. Repo `docs/nginx-setup/mtapi-tscopier.conf` geo matches the live VPS at time of enablement.
- **Tests/verification:** Frontend `tsc -b` and `npm run build` pass; worker `tsc` passes. Live: worker logs show successful `CheckConnect` → `ConnectByToken` → `AccountSummary` (all HTTP 200) for a test MTAPI account; endpoint `/health` returns `Healthy`.
- **Deploy state:** `mtapi-broker` and `fxsocket-broker` deployed to the migration Supabase project (`supmsgcubipmmowrzoub`). Worker changes committed to `migration` and observed running live. VPS nginx config rebuilt and reloaded. Latest commit `fd707e46`, pushed to `origin/migration` and `upstream/migration`.
- **Follow-ups:**
  1. End-to-end UI retest: add an MTAPI account, refresh the page, confirm it appears in the list, the health modal shows status, and balance populates.
...
- **Fix (files):**
  ...
  - `src/lib/brokerLink.ts` — added `resolveProvider`, `hasMtapiBrokerSession`, `hasAnyBrokerSession`. `isBrokerCopyEnabled` and `countLinkedBrokerSessions` use `hasAnyBrokerSession` (needs `mtapi_session_id` — correct for server/full rows). **Note (2026-09-23):** browser rows from `BROKER_ACCOUNT_CLIENT_SELECT` do **not** include `mtapi_session_id`, so client UI must use `hasLinkedBrokerForUi` / `isBrokerCopyEnabledForUi` / `countLinkedBrokerSessionsForUi` instead (added later with the Trades fan-out work).
