...
- **Deploy state:** code written on `migration`; **not yet deployed**. Order: set `MTAPI_INTERNAL_TOKEN` on Supabase + Railway → deploy edge/worker → enable nginx token check + disable IP allowlist → reload. WireGuard is optional and must not be enabled before `wg0` exists.
- **Follow-ups:** re-run Trades smoke test (XAUUSD) after nginx cutover; Railway userspace WireGuard feasibility; password rotation still open (separate incident).
...
- **Follow-ups:** user rotates broker password for `476205231` and reconnects; confirm new logs have no plaintext password; long-term MTAPI log verbosity / upstream redaction; ~~re-enable IP allowlist~~ **done 2026-09-24** (watch for new `403 IP not allowed` if Railway/Supabase egress changes — add `/32` and reload); same image will host production sessions — demo leak is a process warning, not “demo-only”.
...
- **Security:** Added a redacted nginx log format (`mtapi_safe`) that logs `$request_method $uri` without the query string, so broker passwords passed as query parameters never reach disk. Verified: `/ConnectByToken` and `/AccountSummary` appear in `mtapi-access.log` with no `?args`. **IP allowlist ENABLED 2026-09-24** after discovering real worker egress from the access log (not guessed Railway ranges): `152.55.177.10` (trade worker, UA `node`), `13.57.96.134` / `54.219.72.7` (session sync), `162.220.232.0/24`, plus Railway published ranges. `if ($allowed_ip = 0) { return 403; }` is active. Verified: worker still 200; non-allowlisted IP → `403 IP not allowed`; `/health` remains open (not behind the check). API-key auth, TLS, rate limiting, and scanner-path blocking remain active. Repo `docs/nginx-setup/mtapi-tscopier.conf` geo matches the live VPS.
