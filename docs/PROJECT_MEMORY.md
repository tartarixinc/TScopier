...
- **Security:** Added a redacted nginx log format (`mtapi_safe`) that logs `$request_method $uri` without the query string, so broker passwords passed as query parameters never reach disk. Verified: `/ConnectByToken` and `/AccountSummary` appear in `mtapi-access.log` with no `?args`. The MTAPI server block's IP allowlist is currently **disabled** because the worker's real egress IP (`162.220.232.68`) is not in the guessed Railway ranges and would have been blocked; API-key auth, TLS, rate limiting, and scanner-path blocking remain active.
