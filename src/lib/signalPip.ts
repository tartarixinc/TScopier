 ...
 * (predefined SL/TP, range step, close-worse-entries, move SL after pips).
 *
 * Pips = |price distance| × multiplier (lot-independent).
 * Aligns with TelegramBacktester / backtest display conventions.
 */
...
/** Normalize to 6-letter pair when possible (matches TelegramBacktester). */
...
/** @deprecated Use normalizeSignalSymbol */
export const normalizeBacktestSymbol = normalizeSignalSymbol

/**
 * Multiply raw price distance into "pips".
...
/** Price units per 1 pip (inverse of multiplier). */
...
/** @deprecated Use roundSignalPips */
export const roundBacktestPips = roundSignalPips
...
/** Outcome-based pip result from signal TP/SL levels. */
...
/** @deprecated Use computePipsFromSignalOutcome */
